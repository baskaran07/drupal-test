  ...
  public function showVersionWarning(GetResponseEvent $event) {
    ...
    if ($current_user->hasPermission('administer cms content sync')) {
      ...
      if (!empty($version_mismatch)) {
        $mismatching_flow_labels = '';

        if (count($version_mismatch) >= 2) {
          foreach ($version_mismatch as $flow_id => $flow) {
            $link = Link::fromTextAndUrl(key($flow_id), Url::fromRoute('entity.cms_content_sync_flow.edit_form', ['cms_content_sync_flow' => key($flow_id)], ['absolute' => TRUE]))->toString();
            $mismatching_flow_labels .= $link . ', ';
          }
        }
        else {
          $link = Link::fromTextAndUrl(key($version_mismatch), Url::fromRoute('entity.cms_content_sync_flow.edit_form', ['cms_content_sync_flow' => key($version_mismatch)], ['absolute' => TRUE]))->toString();
          $mismatching_flow_labels = $link;
        }

        $message = new TranslatableMarkup('You have to update the related flow(s) <em>@flows</em> to keep the content synchronization intact. Failing to update the config may break the synchronization and lead to damanged or missing content.', ['@flows' => $mismatching_flow_labels]);
        $messanger->addWarning($message);
      }
    }
  }
