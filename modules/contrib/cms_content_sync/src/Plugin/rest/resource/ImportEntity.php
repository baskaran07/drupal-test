 ...
 *     "https://www.drupal.org/link-relations/create" = "/rest/cms-content-sync-import/{pool}/{entity_type_name}/{bundle_name}/{uuid}"
 *   }
 * )
 */
class ImportEntity extends ResourceBase {
  ...
  public function get($pool_id) {
    ...
    foreach (Flow::getAll() as $flow) {
      foreach ($flow->getEntityTypeConfig() as $definition) {
        ...
        $id = SyncCoreFlowExport::getExternalEntityTypeId(
          $pool->id,
    ...
    $url .= '/' . SyncCorePoolExport::EXTERNAL_PREVIEW_PATH;
    $arguments = [];
    if ($page) {
      $arguments['page'] = $page;
    }
  ...
  protected function enrichPreviewItem(&$item) {
    ...
    $entity_status = EntityStatus::getInfosForEntity($entity_type_name, $uuid);
  ...
  public function post($pool_id, $entity_type_name, $bundle_name, $uuid) {
    ...
    foreach (Flow::getAll() as $flow) {
      foreach ($flow->getEntityTypeConfig() as $definition) {
        ...
        $local_connection_id = SyncCoreFlowExport::getExternalConnectionId(
          $pool->id,
          $pool->getSiteId(),
          $entity_type_name,
          $bundle_name,
    ...
    $url = $pool->getBackendUrl();
    $url .= '/' . SyncCorePoolExport::EXTERNAL_PREVIEW_PATH . '/' . $uuid;
