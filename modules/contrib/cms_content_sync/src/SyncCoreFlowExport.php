...
class SyncCoreFlowExport extends SyncCoreExport {
   ...
   */
  const PREVIEW_CONNECTION_ID = 'drupal_cms-content-sync_preview';
  /**
   ...
   *   pool connection.
   */
  const POOL_DEPENDENCY_CONNECTION_ID = 'drupal-[api.name]-' . SyncCorePoolExport::POOL_SITE_ID . '-[entity_type.name_space]-[entity_type.name]-[entity_type.version]';
  ...
  public static function getInternalCreateItemUrl($api_id, $entity_type_name, $bundle_name, $version) {
    return SyncCorePoolExport::getInternalUrl($api_id, $entity_type_name, $bundle_name, $version);
  }
  ...
  public static function getInternalUpdateItemUrl($api_id, $entity_type_name, $bundle_name, $version) {
    return SyncCorePoolExport::getInternalUrl($api_id, $entity_type_name, $bundle_name, $version, '[id]');
  }
  ...
  public static function getInternalReadListUrl($api_id, $entity_type_name, $bundle_name, $version) {
    return SyncCorePoolExport::getInternalUrl($api_id, $entity_type_name, $bundle_name, $version, self::READ_LIST_ENTITY_ID);
  }
    ...
    foreach ($this->flow->getEntityTypeConfig() as $id => $type) {
      ...
      foreach ($entity_type_pools as $pool_id => $definition) {
        ...
        $entity_type = [
          ...
          'api_id' => $api . '-' . SyncCorePoolExport::CUSTOM_API_VERSION,
        ];
        ...
        foreach ($fields as $key => $field) {
          ...
          if (!$field_definition->isReadOnly() || $key == 'menu_link') {
        ...
        $pool_connection_id = self::getExternalConnectionId($api, SyncCorePoolExport::POOL_SITE_ID, $entity_type_name, $bundle_name, $version);
        ...
        $operations[] = [$url . '/api_unify-api_unify-connection-0_1', [
          'json' => [
            ...
            'hash' => self::getExternalConnectionPath($api, SyncCorePoolExport::POOL_SITE_ID, $entity_type_name, $bundle_name, $version),
            'usage' => 'EXTERNAL',
            'status' => 'READY',
            'options' => [
