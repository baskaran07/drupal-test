...
class ExportIntent extends SyncIntent {
  ...
  public function getExternalUrl($uuid) {
    $url = $this->pool->getBackendUrl() . '/' . SyncCoreFlowExport::getExternalConnectionPath(
        $this->pool->id,
        $this->pool->getSiteId(),
        $this->entityType,
        $this->bundle,
  ...
  public function execute() {
            ...
            try {
              ...
              foreach ($flows as $flow) {
                ...
                foreach ($flow->getEntityTypeConfig($embed_entity->getEntityTypeId(), $embed_entity->bundle())['export_pools'] as $pool_id => $behavior) {
                  ...
                  $definition[SyncIntent::SOURCE_CONNECTION_ID_KEY] = SyncCoreFlowExport::getExternalConnectionId(
                    $pool->id,
                    $pool->getSiteId(),
                  ...
                  $definition[SyncIntent::POOL_CONNECTION_ID_KEY] = SyncCoreFlowExport::getExternalConnectionId(
                    $pool->id,
                    SyncCorePoolExport::POOL_SITE_ID,
                    $embed_entity->getEntityTypeId(),
                    $embed_entity->bundle(),
                    $version
                  );
    ...
    \Drupal::logger('cms_content_sync')->info('@not EXPORT @action @entity_type:@bundle @uuid @reason: @message', [
      '@reason' => $reason,
      '@action' => $action,
